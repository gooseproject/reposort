...
querytags = [ 'name', 'version', 'release', 'epoch', 'arch', 'summary',
              ...
              'relativepath', 'hdrstart', 'hdrend', 'id', 'buildorder',
            ]
...
class pkgQuery:
    ...
    of fmt_name), or to self.pkg.returnSimple(), allowing subclasses
    ...
    def fmt_build_order(self, **kw):
        return self.yb.build_order(self.name)
...
class YumBaseQuery(yum.YumBase):
    ...
    def build_order(self, name, **kw):
        """
        This is a function to generate a build order for source packages.
        For binary packages this will give an order of installation.
        Code for build_order came from require (above) and my version of
        ...
        def require_recursive(pkg):
            ...
            if pkg.name in pkgScore:
                pkgScore[pkg.name] += 1
            else:
                pkgScore[pkg.name] = 1

            #find what is required for the given package, then find what
            #  provides that requirement, increment the counter and then
            #  recursively perform that task.
            for dep in pkg.prco("requires"):
              for prov in self.fmt_whatprovides(dep):
                if prov.name in pkgScore:
                    pkgScore[prov.name] = pkgScore[prov.name] + pkgScore[pkg.name] + 1
                else:
                    pkgScore[prov.name] = 1
                require_recursive(prov)
        ...
        """
        The package in question should never come before its requirements.
        This was necessary when I was testing bash.
        """
        pkgScore[pkg.name] = 1
        """
        Sort the package score dictionary in revers order so the most
        required pacakge is installed/built first.  I should probably
        return the sorted dictionary, but I'll just print it.
        """
        for key in sorted(pkgScore, key=pkgScore.__getitem__, reverse=True):
            print key
...
def main(args):
    ...
    parser.add_option("--build-order", action="store_true", dest="build_order",
                      help="identify ideal build order --build-order")
    ...
    if opts.build_order:
        if opts.resolve:
            sackops.append("build_order")
        else:
            pkgops.append("build_order")
    if opts.provides:
